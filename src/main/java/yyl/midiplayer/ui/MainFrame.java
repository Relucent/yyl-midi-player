...
public class MainFrame extends JFrame {
	...
	private void initListener() {
		...
		contentPane
				.setDropTarget(new DropTarget(contentPane, DnDConstants.ACTION_COPY_OR_MOVE, new DropTargetAdapter() {
					@Override
					public void drop(DropTargetDropEvent event) {
						try {
							...
							if (transferable.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
								...
								List<File> fileList = List.class
										.cast(transferable.getTransferData(DataFlavor.javaFileListFlavor));
								...
								// // 显示文件名称
								// // JOptionPane.showMessageDialog(contentPane, file.getAbsolutePath(), "File",
								// JOptionPane.PLAIN_MESSAGE);
								// }
								// 该方法用于通知系统拖放操作的完成结果。当 success 参数为 true 时，表示拖放操作成功完成；当 success 参数为 false
								// 时，表示拖放操作未成功完成或被取消。
